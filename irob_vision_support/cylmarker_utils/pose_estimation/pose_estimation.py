"""Cylmarker pose estimation workflow"""
...
def show_sgmntd_bg_and_fg(im, mask_marker_bg, mask_marker_fg):
    ...
    mask_marker_bg = cv2.subtract(mask_marker_bg, mask_marker_fg)
    mask_bg_blue = np.zeros_like(im_copy)
    mask_bg_blue[:,:,0] = mask_marker_bg
    im_copy = cv2.addWeighted(im_copy, 1.0, mask_bg_blue, alpha, 0)
    # Then we show the foreground part
    alpha = 0.7
    mask_fg_red = np.zeros_like(im_copy)
    mask_fg_red[:,:,2] = mask_marker_fg
    im_copy = cv2.addWeighted(im_copy, 1.0, mask_fg_red, alpha, 0)
    cv2.imshow("Segmentation | Blue: background | Red: foreground", im_copy)
    cv2.waitKey(0)
    ...
    for sqnc in pttrn.list_sqnc:
        if sqnc.sqnc_id != -1:
            ...
            for kpt in sqnc.list_kpts:
                cntr = kpt.cntr
                cv2.drawContours(im, [cntr], -1, blue, 1)
            """ draw line between first and last kpt """
            u_0, v_0 = sqnc.list_kpts[0].get_centre_uv()
            u_1, v_1 = sqnc.list_kpts[-1].get_centre_uv()
            im = cv2.line(im, (int(u_0), int(v_0)), (int(u_1), int(v_1)), yellow, 2, cv2.LINE_AA) # lines
            """ draw centroids """
            for kpt in sqnc.list_kpts:
                u, v = kpt.get_centre_uv()
                im = cv2.circle(im, (int(round(u)), int(round(v))), radius=2, color=red, thickness=-1)
    return im
    #cv2.waitKey(0)
...
def show_axis(im, transf, rvecs, tvecs, cam_matrix, dist_coeff, length_m):
    ...
    for i in args:
        cv2.line(im, frame_centre, tuple(np.rint(imgpts[i+1]).astype(int).ravel()), colors[i], thickness, cv2.LINE_AA)
    return im
...
def show_reproj_error_image(im, pts2d_filtered, pts2d_projected):
    ...
    for pt2d_d, pt2d_p in zip(pts2d_filtered[:,0], pts2d_projected[:,0]):
        ...
        im = cv2.line(im, pt2d_d, pt2d_p, color=red, thickness=1, lineType=cv2.LINE_AA)
        im = cv2.circle(im, pt2d_d, radius=1, color=red, thickness=-1)
        im = cv2.circle(im, pt2d_p, radius=1, color=green, thickness=-1)
    cv2.imshow("image", im)
    cv2.waitKey(0)
...
def get_reprojection_error(pts3d, rvec, tvec, inliers, cam_matrix, dist_coeff, pnts2d, show_reproj_error, im):
    ...
    pts2d_projected, jacobian = cv2.projectPoints(pts3d_filtered, rvec, tvec, cam_matrix, dist_coeff)
    ...
    for sqnc in pttrn.list_sqnc:
        if sqnc.sqnc_id != -1:
            ...
            for kpt in sqnc.list_kpts:
                ...
                cntr = kpt.cntr
                #im = cv2.drawContours(im, [cntr], -1, [0, 255, 0], -1)
                im = cv2.drawContours(im, [cntr], -1, [0, 255, 0], 1)
                # Then, we calculate + draw the projected contour (in red)
                corners_3d = np.float32(kpt.xyz_corners).reshape(-1,3)
                imgpts, jac = cv2.projectPoints(corners_3d, rvecs, tvecs, cam_matrix, dist_coeff)
                imgpts = np.asarray(imgpts, dtype=np.int32)
                #im = cv2.fillPoly(im, [imgpts], [0, 0, 255])
                im = cv2.polylines(im, [imgpts], True, [0, 0, 255], thickness=1, lineType=cv2.LINE_AA)
    return im


def estimate_poses(image, cam_calib_data, config_file_data, data_pttrn, data_marker, debug_im_path_stem = None):
    """Estimates the position of the marker, giving the pose of the pole near the ring of binary ones on the marker.
    """
    if image is None:
        raise Exception("Empty image provided.")
    
    if debug_im_path_stem is not None:
        os.makedirs(debug_im_path_stem, exist_ok=True)
        cv2.imwrite(os.path.join(debug_im_path_stem, "read_image.jpg"), image)
    ...
    im = cv2.undistort(image, cam_matrix, dist_coeff_np) # undistort each new image
    dist_coeff = None # we don't need to undistort again the same image

    if debug_im_path_stem is not None:
        cv2.imwrite(os.path.join(debug_im_path_stem, "undistorted_img.jpg"), im)
    ...
    if debug_im_path_stem is not None:
        cv2.imwrite(os.path.join(debug_im_path_stem, "mask_marker_bg.jpg"), mask_marker_bg)
        cv2.imwrite(os.path.join(debug_im_path_stem, "mask_marker_fg.jpg"), mask_marker_fg)
    ...
    if debug_im_path_stem is not None:
        ...
        cv2.imwrite(os.path.join(debug_im_path_stem, "lines_countours_image.jpg"), lines_countours_image)
    ...
    valid, rvec_pred, tvec_pred, inliers = cv2.solvePnPRansac(pnts_3d_object, pnts_2d_image, cam_matrix, dist_coeff, None, None, False, 1000, 3.0, 0.9999, None, cv2.SOLVEPNP_SQPNP)
    if not valid:
        return None
    ...
    if debug_im_path_stem is not None:
        ...
        cv2.imwrite(os.path.join(debug_im_path_stem, "features_image.jpg"), features_image)
    ...
    if debug_im_path_stem is not None:
        ...
        cv2.imwrite(os.path.join(debug_im_path_stem, "axis_image.jpg"), axis_image)
